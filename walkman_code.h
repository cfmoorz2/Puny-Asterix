...
Object flashlight "flashlight" mabel
    with 
        name 'flashlight' 'light' 'torch' 'black' 'plastic',
        description [;
...
Object eject_button "eject button" walkman
    ...
    has scenery;
Object dummy_eject_button "eject button" dummy_walkman
    with name 'eject' 'button', 
        article "an",
        description"It's a chunky black button with the 'eject' symbol on the top. ",
        before [;
...
Object play_button "play button" walkman
    ...
    has scenery;  
Object dummy_play_button "play button" dummy_walkman
    with name 'play' 'button',
        description"It's a chunky black button with a 'play' arrow on the top. ",
        before [ obj;
            push:
                ...
                "With a hum, the little rotors start to turn. ";
        ],
    has scenery;   
...
Object stop_button "stop button" walkman
    ...
    has scenery;
Object dummy_stop_button "stop button" dummy_walkman
    with name 'stop' 'button',
        description"It's a chunky black button with the 'stop' square on the top. ",
        before [;
            push:
...
Object fast_forward_button "fast forward button" walkman
    ...
    has scenery; 
Object dummy_fast_forward_button "fast forward button" dummy_walkman
    with name 'fast' 'forward' 'button',
        description"It's a chunky black button with two 'FF' arrows on the top. ",
        before [ obj;
            push:
...
Object rewind_button "rewind button" walkman
    ...
    has scenery; 
Object dummy_rewind_button "rewind button" dummy_walkman
    with name 'rewind' 'button',
        description"It's a chunky black button with two backwards 'rewind' arrows on the top.",
        before [ obj;
            push:
...
Class Tape 
    ...
    tape_advance 0,
    fast_forward [ i;
        if (TestScope(walkman)) walkman_playing = false;
        if (TestScope(dummy_walkman)) dummy_walkman_playing = false;
        self.tape_advance = 0;
    ...
    press_play [;
        ...
        if (TestScope(walkman)) walkman_playing = true;
        if (TestScope(dummy_walkman)) dummy_walkman_playing = true;
        ],
    play [;
        if (self.current_track == SIDE_END) "The tape seems to be at the end of the side. ";
        if (self.current_track == SIDE_START) self.current_track = FIRST_TRACK;
    ...
    before [ x;
        flip:
            ...
            if (TestScope(walkman)) give walkman ~open;
            if (TestScope(dummy_walkman)) give dummy_walkman_playing ~open;
...
Tape mixtape "casette tape with a yellow label" walkman
    with name 'mix' 'mixtape' 'tape' 'cassette' 'yellow',
        ...
        each_turn [;
            if(self in walkman && walkman_playing && TestScope(self)) self.playback();
        ],
        has item;
